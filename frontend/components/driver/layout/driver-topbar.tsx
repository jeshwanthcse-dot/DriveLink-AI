...
/**
 * DriverTopbar — Reusable top navigation panel for the Driver portal
 * Contains responsive menu triggers, breadcrumbs, mock search, notifications, theme toggle, and status controls
 */

import { useState } from "react";
import { Menu, Search, Sun, Moon, Bell, CheckCircle2, AlertCircle, ShieldAlert } from "lucide-react";
import { DriverBreadcrumb } from "../navigation/driver-breadcrumb";
import { Avatar } from "@/components/common/avatar";
import { NotificationIcon } from "@/components/ui/notification-icon";
import { MOCK_DRIVER } from "@/constants/driver-nav";
...
export function DriverTopbar({
  ...
  const notifications = React.useMemo(() => {
    return allNotifications.filter((n) => n.portal === "driver");
  }, [allNotifications]);
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="relative inline-block text-left">
          <button
            ...
            className={cn(
              "flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-semibold border transition-all active:scale-[0.98]",
              status === "available"
          ...
          >
            ...
            />
            <span className="capitalize">{status === "available" ? "Online" : "Offline"}</span>
          </button>
        </div>
            ...
            <>
              ...
              <div className="absolute right-0 mt-2.5 z-50 w-72 origin-top-right rounded-2xl border border-border bg-card p-4 shadow-elevated">
                <div className="flex items-center justify-between border-b pb-2 mb-2">
                  ...
                  {unreadCount > 0 && (
                    <button
                      onClick={() => markNotificationsAsRead("driver")}
        ...
        {/* User avatar with status summary */}
        <div className="flex items-center gap-2 border-l pl-3">
          <Avatar initials={MOCK_DRIVER.avatarInitials} size="sm" />
          <div className="hidden text-left xl:block">
            <p className="text-xs font-semibold leading-tight">{MOCK_DRIVER.name}</p>
